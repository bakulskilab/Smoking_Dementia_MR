...
eligible_group = final_eligible %>% mutate(group = "eligible")
exclude_group = smk_status %>% select(names(final_eligible)) %>% 
  anti_join(eligible_group) %>% 
  mutate(group = "exclude")
full_combine = rbind(eligible_group, exclude_group) %>% as.data.frame()
full_combine_nolabel = remove_all_labels(full_combine) %>% 
...
table_inexc = descrTable(group ~ age + sex + gen_ancestry + education + proxy + 
                       ...
                       full_combine_nolabel, show.all = T)
compareGroups::export2csv(table_inexc, 
                          file = paste0(output_path, 'in_exclude.csv'))
```
