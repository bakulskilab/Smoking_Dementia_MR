...
cog_rand_clean = hrs_rand_combine %>% 
  ...
  select(HHID, PN, last_cog_wave, birth_year, age, gender, race, ethnicity, education, proxy, 
         cog_last_visit, ever_smoke, current_smoke, ever_drink, bmi, cesd, hypertension, diabetes, stroke)
...
save(cog_smk_combined, file = paste0(work_data_path, "clean_df/cog_smk_combined_0124.rda"))
```
...
```{r}
AD_sig = read.table(file = paste0(work_data_path, "sumstats/sig_snps/AlzheimerDisease.txt"), header = T, sep = "\t", fill = T)
...
# Write output for snp extraction on server
write.table(name_out_AD, file = paste0(work_data_path, "sumstats/sig_snps/candidate_AD_wdup.txt"),
            sep = "\t", quote = F, row.names = F, col.names = F)
```
...
fam.order = as.data.table(obj.bigSNP$fam)
# Get betas from sumstats
AD_fullsig = read.table(file = paste0(work_data_path, "sumstats/sig_snps/AD_snps.txt"), header = T, sep = "\t", fill = T)
keep_id = map %>% dplyr::select(chr, pos)
AD_keep = AD_fullsig %>% dplyr::select(chr, pos, non_effect_allele, effect_allele, beta, se, gwasP) %>% right_join(keep_id) %>% 
  dplyr::rename(a1 = effect_allele, a0 = non_effect_allele)
...
hrs_short_prs = hrs_short_raw %>% 
  ...
  select(HHID, PN, STUDY, APOE2010_bin, AA_PGS4_SI_GSCAN19, EA_PGS4_SI_GSCAN19, AA_PGS3_EVRSMK_TAG10, EA_PGS3_EVRSMK_TAG10, 
         gen_ancestry, EAPH13_KunkleYAPOE_PT_0_01, AAPH13_KunkleYAPOE_PT_0_01)
...
eur_AD_mean = cross_gen_norm %>% filter(gen_ancestry == "European") %>% pull(prs_AD) %>% mean()
eur_AD_sd = cross_gen_norm %>% filter(gen_ancestry == "European") %>% pull(prs_AD) %>% sd()
afr_AD_mean = cross_gen_norm %>% filter(gen_ancestry == "African") %>% pull(prs_AD) %>% mean()
afr_AD_sd = cross_gen_norm %>% filter(gen_ancestry == "African") %>% pull(prs_AD) %>% sd()
...
pgs_norm = cross_gen_norm %>% 
  ...
  select(HHID, PN, STUDY, gen_ancestry, APOE2010_bin, hrs_SI_GSCAN19, hrs_SI_TAG10, hrs_AD_Kunkle19,
         prs_SI_norm, prs_CPD_norm, prs_AI_norm, prs_SC_norm, prs_AD_norm)
...
final_sample_pre = combined_gen %>% inner_join(pgs_norm) %>% 
  select(HHID, PN, STUDY, last_cog_wave, age, gender, education, proxy, gen_ancestry, cog_last_visit, 
         hrs_SI_GSCAN19, hrs_SI_TAG10, hrs_AD_Kunkle19, APOE2010_bin, prs_SI_norm, prs_CPD_norm, prs_AI_norm, prs_SC_norm, prs_AD_norm,
         ever_smoke, current_smoke, cig_now, age_start_smk, yrs_since_start, age_stop_smk, yrs_since_stop, total_smk_yr, cig_most,
         ever_drink, bmi, cesd, hypertension, diabetes, stroke) %>% 
  dplyr::rename(sex = gender, cpd_now = cig_now)
# Also add PCs
hrs_pcs = hrs0910 %>% select(HHID, PN, PC1_5A, PC1_5B, PC1_5C, PC1_5D, PC1_5E)
final_sample_pre = final_sample_pre %>% 
  left_join(hrs_pcs)
